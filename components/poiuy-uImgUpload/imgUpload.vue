		...
		methods: {
			...
			asyncUpload(index) {
				...
				if (this.imgArray.length) {
					...
					_this.prompt = "";
					for (let i = index; i < this.imgArray.length; i++) {
						let item = this.imgArray[i];
						uni.uploadFile({
							url: _this.url,
							filePath: item.url,
							name: 'file',
